...
if (rlang::is_installed(c("mlbench", "MASS"))) {
  ...
  prior_spec_lda <- discrim_linear() %>% set_engine("MASS", prior = rep(1/6, 6))

  exp_f_fit_lda     <- MASS::lda(Type ~ ., data = glass_tr)
  exp_xy_fit_lda    <- MASS::lda(x = glass_tr[,-10], grouping = glass_tr$Type)
  exp_prior_fit_lda <- MASS::lda(Type ~ ., data = glass_tr, prior = rep(1/6, 6))
  ...
  exp_xy_fit_qda <- MASS::qda(x = penguin_tr[, -1], grouping = penguin_tr$species)
  exp_prior_fit_qda <- MASS::qda(species ~ ., data = penguin_tr, prior = rep(1 / 3, 3))
...
if (rlang::is_installed(c("mlbench", "klaR"))) {
  ...
  prior_spec_rda <- discrim_regularized() %>% set_engine("klaR", prior = rep(1 / 6, 6))
...
if (rlang::is_installed(c("mlbench", "mda", "earth"))) {
  ...
  exp_f_fit_fda <- mda::fda(Type ~ ., data = glass_tr, method = earth::earth, nprune = 7)
...
if (rlang::is_installed(c("mlbench", "mda"))) {
  ...
  exp_f_fit_lda_fda <- mda::fda(Type ~ ., data = glass_tr, method = mda::gen.ridge, lambda = 1)

  exp_f_wts_fit_lda_fda <- mda::fda(Type ~ ., data = glass_tr, weights = as.double(wts),
